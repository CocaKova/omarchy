...
If your agent lives somewhere other than this machine — a workstation, a GPU box, a Spark on your Tailscale network — point Omarchy at it under _Setup > Defaults > Agent Host_, or with `omarchy default agent --host gpu-box`. From then on `Super + Shift + Ctrl + A`, `omarchy agent`, and `omarchy agent prompt "Review this project"` all open that agent over SSH instead of locally, in the same dedicated terminal window, with the same don't-stop-to-ask flags and the same initial prompt as it would get here. The host is whatever SSH understands, so `user@host` works too, and key-based authentication is what keeps a keypress feeling like a keypress. `omarchy default agent --host` on its own prints the current one, and `omarchy default agent --host ""` brings the agent back to this machine.

The far side starts the agent through a login shell, so a mise-managed agent over there is found the same way it is here, and the session starts in `~/Work` on that machine for the same reason a local one does. Omarchy also stops managing the installation once a host is set, because the agent lives on the other machine now: choosing one records the choice and launches it without installing anything locally.

The agents panel keeps working, with one half of it narrowed: rate limits and prepaid balances are account-wide, so they still account for everything the remote agent spends, and right-clicking the bar icon launches it over there like any other launch. The token charts are the part that only knows this machine, since they are scanned from the transcripts each agent leaves on the disk it ran on. If you want those to cover both machines, turn on the panel's `syncMode` and point `syncDir` at a folder both of them sync.

If the machine can't be reached, Omarchy says so with a notification rather than flashing a terminal window open and shut; run the agent from a terminal and it tells you there instead. The check is a plain connection test, so a host that asks for a password or a key passphrase still counts as reachable, and a jump host or `ProxyCommand` skips the check and lets SSH do its own work.

Work that is about this machine stays on it. Crash diagnosis is the one that matters — the core dump, the crashed binary and the skill are all here — so `omarchy agent crash` runs the agent locally even when it normally runs elsewhere, and says so if the agent isn't installed here. Anything else can be pinned the same way with `omarchy agent --local`.
